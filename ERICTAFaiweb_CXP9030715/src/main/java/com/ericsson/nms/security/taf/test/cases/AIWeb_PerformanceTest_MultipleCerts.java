...
public class AIWeb_PerformanceTest_MultipleCerts extends TorTestCaseHelper implements TestCase {
    ...
    @BeforeSuite
    void setup() {
        WriteFilesToServer.copyRequiredFilesToServer(HostConfigurator.getSC2());
    }
    ...
    @DataDriven(name = "aiweb_regression_downloadfilePerformanceMultipleCerts")
    @Test(groups = { "{Acceptance", "aiweb_regression_downloadfileSuccess}" })
    public void downloadTheFileUsingTheSameLogicalNameSerialCombinations_MultipleCerts(@Input("host") final String hostIn,
